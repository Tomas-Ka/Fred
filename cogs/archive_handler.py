...
class PersistentArchiveRoleView(discord.ui.View):
    ...
    #set up the button and the callback function (the callback function is the function that is run when the button is pressed)
    @discord.ui.button(label='Toggle archives', style=discord.ButtonStyle.grey, custom_id='archive_access_button')
    async def archive(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if role in interaction.user.roles:
            ...
            #Id the user doesn't have the role, add it to the player and send confirm message
...
class ArchiveHandler(commands.Cog):
    ...
    #command to send the archive join message (/join_arhive), should be locked to admin role
    ...
    async def join_archive(self, interaction: discord.Interaction) -> None:
        embed=discord.Embed(title="The Archives", description="If you want to see all our old chatlogs from previous years, they are all available in our archive that is hidden by default. To view it, just press the button bellow this message!")
