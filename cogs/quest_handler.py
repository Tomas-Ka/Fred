...
class SetQuestAmount(discord.ui.Modal, title="Set Quests Played"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        if isinstance(self.player.value, int):
            db.update_player(user.id, self.player.value)
            await interaction.response.send_message(f"Updated amount of quests for player {user.display_name} to be {self.player.value}")
        else:
